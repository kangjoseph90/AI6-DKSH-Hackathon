...
class DQN:
    def __init__(self, episode):
        ...
        self.model = nn.Sequential(
            nn.Linear(in_features=6, out_features=self.batch_size),
            nn.ReLU(),
            nn.Linear(in_features=self.batch_size, out_features=3)
    	)

        self.optimizer = optim.Adam(params=self.model.parameters(), lr=self.lr)
        self.criterion = nn.MSELoss()
        self.steps_done = 0
        self.deque_size = int(1e3)
    ...
    def optimize_model(self):
        ...
        states = torch.cat(states).reshape(self.batch_size, 6)
        next_states = torch.cat(next_states).reshape(self.batch_size, 6)
